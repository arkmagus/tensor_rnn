...
from ...modules.tucker import TuckerLinear
from ...utils.helper import torchauto, tensorauto

class StatefulTuckerGRUCell(StatefulBaseCell) :
    def __init__(self, in_modes, out_modes, ranks, bias=True, cache=True,
        ...
        if compress_in :
            self.weight_ih = TuckerLinear(in_modes, self.out_modes_Mx, ranks, bias=self.bias, cache=cache)
        else :
            self.weight_ih = nn.Linear(self.input_size, self.hidden_size*3, bias=self.bias)
        if compress_in :
            self.weight_hh = TuckerLinear(out_modes, self.out_modes_Mx, ranks, bias=self.bias, cache=cache)
        else :
...
class StatefulTuckerLSTMCell(StatefulBaseCell) :
    def __init__(self, in_modes, out_modes, ranks, bias=True, cache=True,
            compress_in=True, compress_out=True) :
        super().__init__()
        self.in_modes = in_modes
        ...
        if compress_in :
            ...
            self.weight_hh = nn.Linear(self.input_size, self.hidden_size*4, bias=self.bias)
        if compress_out :
            self.weight_hh = TuckerLinear(out_modes, self.out_modes_Mx, ranks, bias=self.bias, cache=cache)
        else :
